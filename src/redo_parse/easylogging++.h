...
#define ELPP_COUT_LINE(logLine) logLine << std::flush
    ...
    /// @brief Internal helper class that makes all default constructors
    /// private.
    ///
  ...
  /// @brief Represents enumeration for severity level used to determine level
  /// of logging
  ///
  ...
  enum class Level : base::type::EnumType {
    ...
    Fatal = 8,
    /// @brief Information representing errors in application but application
    /// will keep running
    Error = 16,
    /// @brief Useful when application has potentially harmful situtaions
    Warning = 32,
    /// @brief Information that can be highly useful and vary with verbose
    /// logging level.
    Verbose = 64,
  ...
  class LevelHelper : base::StaticClass {
   public:
    /// @brief Represents minimum valid level. Useful when iterating through
    /// enum.
  ...
  enum class ConfigurationType : base::type::EnumType {
    /// @brief Determines whether or not corresponding level and logger of
    /// logging is enabled
    ...
    MaxLogFileSize = 128,
    /// @brief Specifies number of log entries to hold until we flush pending
    /// log data
    LogFlushThreshold = 256,
    /// @brief Represents unknown configuration
    Unknown = 1010
  };
  /// @brief Static class that contains helper functions for
  /// el::ConfigurationType
  class ConfigurationTypeHelper : base::StaticClass {
   public:
    /// @brief Represents minimum valid configuration type. Useful when
    ...
    /// @brief Casts configuration type to int, useful for iterating through
    /// enum.
    ...
    /// @brief Applies specified function to each configuration type starting
    /// from startIndex
    /// @param startIndex initial value to start the iteration from. This is
  ...
  enum class LoggingFlag : base::type::EnumType {
    ...
    /// @brief Flushes log with every log-entry (performance sensative) -
    /// Disabled by default
    ImmediateFlush = 16,
  ...
  namespace base {
    ...
    namespace consts {
      // Level log values - These are values that are replaced in place of
      // %level format specifier
      ...
      const struct {
        ...
        const char* detail;
            ...
            {SIGFPE, "SIGFPE", "Erroneous arithmetic operation",
             "Arithemetic operation issue such as division by zero or "
             "operation resulting in overflow."},
            {SIGILL, "SIGILL", "Illegal instruction",
             "Generally due to a corruption in the code or to an attempt to "
             "execute data."},
  ...
  namespace base {
    ...
    namespace utils {
      ...
      /// @brief Gets value of const char* but if it is nullptr, a string
      /// nullptr is returned
      ...
      /// @brief Bitwise operations for C++11 strong enum class. This casts e
      /// into Flag_T and returns value after bitwise operation
      /// Use these function as <pre>flag = bitwise::Or<MyEnum>(MyEnum::val1,
      /// flag);</pre>
      namespace bitwise {
        template <typename Enum>
        static inline base::type::EnumType And(Enum e,
                                               base::type::EnumType flag) {
          return static_cast<base::type::EnumType>(flag) &
                 static_cast<base::type::EnumType>(e);
        }
        template <typename Enum>
        static inline base::type::EnumType Not(Enum e,
                                               base::type::EnumType flag) {
          return static_cast<base::type::EnumType>(flag) &
                 ~(static_cast<base::type::EnumType>(e));
        }
        template <typename Enum>
        static inline base::type::EnumType Or(Enum e,
                                              base::type::EnumType flag) {
          return static_cast<base::type::EnumType>(flag) |
                 static_cast<base::type::EnumType>(e);
        }
    ...
    namespace threading {
      ...
      namespace internal {
        ...
        /// @brief Scoped lock for compiler that dont yet support
        /// std::lock_guard
        template <typename M>
    ...
    namespace utils {
      class File : base::StaticClass {
       public:
        ...
        static base::type::fstream_t* newFileStream(
        ...
        static inline bool pathExists(const char* path,
          ...
          return considerFile
                     ? true
                     : ((fileType & FILE_ATTRIBUTE_DIRECTORY) == 0 ? false
                                                                   : true);
#endif  // ELPP_OS_UNIX
        }
      ...
      /// @brief String utilities helper class used internally. You should not
      /// use it.
      class Str : base::StaticClass {
       public:
        ...
        /// @return Returns true if starts with specified string, false
        /// otherwise
        ...
        static inline char* convertAndAddToBuff(std::size_t n, int len,
                                                char* buf, const char* bufLim,
        ...
        static inline char* clearBuff(char buff[], std::size_t lim) {
          STRCPY(buff, "", lim);
          ELPP_UNUSED(lim);  // For *nix we dont have anything using lim in
                             // above STRCPY macro
          return buff;
        }
      ...
      /// @brief Operating System helper static class used internally. You
      /// should not use it.
      class OS : base::StaticClass {
       public:
        ...
        /// @brief Gets environment variable. This is cross-platform and CRT
        /// safe (for VC++)
        /// @param variableName Environment variable name
        /// @param defaultVal If no environment variable or value found the
        /// value to return by default
        /// @param alternativeBashCommand If environment variable not found what
        /// would be alternative bash command
        ///        in order to look for value user is looking for. E.g, for
        ///        'user' alternative command will 'whoami'
        ...
        ///
        /// @detail For android systems this is device name with its
        /// manufacturer and model seperated by hyphen
...
#define ELPP_INITI_BASIC_DECLR                                               \
  namespace el {                                                             \
    namespace base {                                                         \
      ...
      /// @brief Contains utilities for cross-platform date/time. This class
      /// make use of el::base::utils::Str
      class DateTime : base::StaticClass {
       public:
        /// @brief Cross platform gettimeofday for Windows and unix platform.
        /// This can be used to determine current millisecond.
        ///
        /// @detail For unix system it uses gettimeofday(timeval*, timezone*)
        /// and for Windows, a seperate implementation is provided
        ...
        /// @param msWidth A pointer to base::MillisecondsWidth from
       ...
       private:
        static inline struct ::tm* buildTimeInfo(struct timeval* currTime,
          ...
          return timeInfo;
#else
          // For any other compilers that don't have CRT warnings issue e.g,
          // MinGW or TDM GCC- we use different method
          time_t rawTime = currTime->tv_sec;
        ...
        static char* parseFormat(char* buf, std::size_t bufSz,
                                 const char* format, const struct tm* tInfo,
                                 std::size_t msec,
            ...
            if (*format == base::consts::kFormatSpecifierChar) {
              switch (*++format) {
                ...
                case '\0':  // End
                  ...
                  break;
                case 'd':  // Day
                  buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_mday, 2,
                                                              buf, bufLim);
                  continue;
                ...
                case 'A':  // Day of week (long)
                  ...
                  continue;
                case 'M':  // month
                  buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_mon + 1,
                                                              2, buf, bufLim);
                  continue;
                case 'b':  // month (short)
                  buf = base::utils::Str::addToBuff(
                ...
                case 'h':  // hour (12-hour)
                  ...
                  continue;
                case 'H':  // hour (24-hour)
                  buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_hour, 2,
                                                              buf, bufLim);
                  continue;
                case 'm':  // minute
                  buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_min, 2,
                                                              buf, bufLim);
                  continue;
                case 's':  // second
                  buf = base::utils::Str::convertAndAddToBuff(tInfo->tm_sec, 2,
                                                              buf, bufLim);
                  continue;
                case 'z':  // milliseconds
                case 'g':
      ...
      class CommandLineArgs {
       public:
        ...
        inline friend base::type::ostream_t& operator<<(
            base::type::ostream_t& os, const CommandLineArgs& c) {
      ...
      /// @brief Abstract registry (aka repository) that provides basic
      /// interface for pointer repository specified by T_Ptr type.
      ///
      /// @detail Most of the functions are virtual final methods but anything
      /// implementing this abstract class should implement
      /// unregisterAll() and deepCopy(const AbstractRegistry<T_Ptr,
      /// Container>&) and write registerNew() method according to container
      /// and few more methods; get() to find element, unregister() to
      /// unregister single entry.
      ...
      /// @brief A pointer registry mechanism to manage memory and provide
      /// search functionalities. (non-predicate version)
      ///
      /// @detail NOTE: This is thread-unsafe implementation (although it
      /// contains lock function, it does not use these functions)
      ...
      class Registry : public AbstractRegistry<T_Ptr, std::map<T_Key, T_Ptr*>> {
       public:
        ...
        /// @brief Copy constructor that is useful for base classes. Try to
        /// avoid this constructor, use move constructor.
       ...
       private:
        virtual inline void deepCopy(const AbstractRegistry<
            T_Ptr, std::map<T_Key, T_Ptr*>>& sr) ELPP_FINAL {
      ...
      /// @brief A pointer registry mechanism to manage memory and provide
      /// search functionalities. (predicate version)
      ///
    ...
    class LogFormat : public Loggable {
     public:
      ...
      void parseFromFormat(const base::type::string_t& userFormat) {
        ...
        if ((dateIndex =
                 formatCopy.find(base::consts::kDateTimeFormatSpecifier)) !=
            std::string::npos) {
          while (dateIndex > 0 &&
     ...
     protected:
      ...
      virtual void updateDateFormat(
          std::size_t index, base::type::string_t& currFormat) ELPP_FINAL {
      ...
      /// @brief Updates %level from format. This is so that we dont have to do
      /// it at log-writing-time. It uses m_format and m_level
      virtual void updateFormatSpec(void) ELPP_FINAL {
        // Do not use switch over strongly typed enums because Intel C++
        // compilers dont support them yet.
  ...
  class Configurations : public base::utils::RegistryWithPred<
                             Configuration, Configuration::Predicate> {
   public:
    ...
    inline Configuration* get(Level level,
                              ConfigurationType configurationType) {
      base::threading::ScopedLock scopedLock(lock());
      return RegistryWithPred<Configuration, Configuration::Predicate>::get(
    ...
    /// @see Configurations::set(Level level, ConfigurationType
    /// configurationType, const std::string& value)
    ...
    ///
    /// @detail If this repository was set manually or by text this returns
    /// empty string.
    ...
    /// configuration for all the configurations except for Enabled or not, we
    /// use this so that ENABLED is set to default i.e,
    /// true. If you dont do this explicitley (either by calling this function
    /// or by using second param in Constructor
    ...
    class Parser : base::StaticClass {
     public:
      /// @brief Parses configuration from file.
      /// @param configurationFile Full path to configuration file
      /// @param sender Sender configurations pointer. Usually 'this' is used
      /// from calling class
      /// @param base Configurations to base new configuration repository off.
      /// This value is used when you want to use
      ///        existing Configurations to base all the values and then set
      ///        rest of configuration via configuration file.
      /// @return True if successfully parsed, false otherwise. You may define
      ...
      /// @brief Parse configurations from configuration string.
      ///
      /// @detail This configuration string has same syntax as configuration
      /// file contents. Make sure all the necessary
      ...
      /// This value is used when you want to use
      ///        existing Configurations to base all the values and then set
      ///        rest of configuration via configuration text.
  ...
  namespace base {
    ...
    /// This is to perform faster while writing logs using correct
    /// configurations.
    ///
      ...
      void build(Configurations* configurations) {
        ...
        for (Configurations::const_iterator it = configurations->begin();
          ...
          if (conf->configurationType() == ConfigurationType::Enabled) {
            ...
            setValue(Level::Global, base::MillisecondsWidth(static_cast<int>(
                                        getULong(conf->value()))),
        ...
        // As mentioned early, we will now set filename configuration in
        // separate loop to deal with non-existent files
        for (Configurations::const_iterator it = configurations->begin();
             it != configurations->end(); ++it) {
          Configuration* conf = *it;
    ...
    /// @brief Class that keeps record of current line hit for occasional
    /// logging
    class HitCounter {
     public:
    ...
    class RegisteredHitCounters
     ...
     public:
      ...
      bool validateAfterN(const char* filename, unsigned long int lineNumber,
        ...
        // Do not use validateHitCounts here since we do not want to reset
        // counter here
  ...
  /// logs
  ///
  /// @detail This class does not write logs itself instead its used by writer
  /// to read configuations from.
  ...
  namespace base {
    ...
    class RegisteredLoggers
     ...
     public:
      ...
      inline void setDefaultConfigurations(
    ...
    class Storage : base::NoCopy, public base::threading::ThreadSafe {
     public:
      ...
      explicit Storage(const LogBuilderPtr& defaultLogBuilder)
        ...
        Logger* sysLogLogger = m_registeredLoggers->get(
...
#define ELPP el::base::elStorage
            ...
            if (fs != nullptr) {
              ...
              ELPP_INTERNAL_ERROR(
                  "Log file for ["
                      ...
                      << "] "
                      << "has not been configured but [TO_FILE] is configured "
                         "to TRUE. [Logger ID: " << logger->id() << "]",
                  false);
            }
          }
        }
  ...
  namespace base {
     ...
     public:
      base::type::string_t build(const LogMessage* logMessage,
        ...
        if (logFormat->hasFlag(base::FormatFlags::File)) {
          ...
          base::utils::Str::replaceFirstWithEscape(
              logLine, base::consts::kLogFileFormatSpecifier,
              std::string(buff));
        }
        ...
        if (logFormat->hasFlag(base::FormatFlags::Line)) {
          ...
          buf = base::utils::Str::convertAndAddToBuff(
              logMessage->line(), base::consts::kSourceLineMaxLength, buf,
              bufLim, false);
          base::utils::Str::replaceFirstWithEscape(
              logLine, base::consts::kLogLineFormatSpecifier,
              std::string(buff));
        }
    ...
    class LogDispatcher : base::NoCopy {
     public:
      ...
      void dispatch(void) {
        ...
        for (const std::pair<std::string, base::type::LogDispatchCallbackPtr>&
                 h : ELPP->m_logDispatchCallbacks) {
    ...
    ///
    /// @detail There is workaround needed to loop through some stl containers.
    /// In order to do that, we need iterable containers
      ...
      inline MessageBuilder& operator<<(
        ...
        base::workarounds::IterablePriorityQueue<T, Container, Comparator>
            iterablePriorityQueue_ =
                static_cast<base::workarounds::IterablePriorityQueue<
                    T, Container, Comparator>>(priorityQueue_);
      ...
      void triggerDispatch(void) {
        if (m_proceed) {
          base::LogDispatcher(m_proceed,
                              LogMessage(m_level, m_file, m_line, m_func,
...
#define ELPP_CURR_FILE_PERFORMANCE_LOGGER el::base::consts::kPerformanceLoggerId
  ...
  namespace base {
    /// @brief Represents performanceTracker block of code that conditionally
    /// adds performance status to log
    ...
    class PerformanceTracker : public base::threading::ThreadSafe,
     ...
     public:
      PerformanceTracker(
        ...
        // the end of scope
        // or before calling checkpoint, we still depend on state of
  ...
  namespace base {
    /// @brief Contains some internal debugging tools like crash handler and
    /// stack tracer
    namespace debug {
              ...
              if (mangName != nullptr && hex != nullptr && addr != nullptr &&
                ...
                *addr++ = '\0';
                ...
                if (status == 0) {
                  ...
                  StackTraceEntry entry(i - 1, strings[i], demangName, hex,
                                        addr);
      ...
      static void logCrashReason(int sig, bool stackTraceIfAvailable,
                                 Level level, const char* logger) {
        std::stringstream ss;
        ss << "CRASH HANDLED; ";
        ss << crashReason(sig);
#if ELPP_STACKTRACE
...
#define ELPP_INITIALIZE_SYSLOG(id, opt, fac) \
  ...
  class Helpers : base::StaticClass {
   public:
    ...
    /// @brief Sets application arguments and figures out whats active for
    /// logging and whats not.
    ...
    static inline void logCrashReason(
      ...
      el::base::debug::logCrashReason(sig, stackTraceIfAvailable, level,
                                      logger);
    }
    /// @brief Installs pre rollout callback, this callback is triggered when
    /// log file is about to be rolled out
    ...
    template <typename T>
    static inline bool installPerformanceTrackingCallback(
        const std::string& id) {
    ...
    /// @brief Returns command line arguments (pointer) provided to
    /// easylogging++
  ...
  class Loggers : base::StaticClass {
   public:
    ...
    /// @brief Unregisters logger - use it only when you know what you are
    /// doing, you may unregister
    ...
    /// @brief Reconfigures single configuration for all the loggers for
    /// specified level
    static inline void reconfigureAllLoggers(
        Level level, ConfigurationType configurationType,
        const std::string& value) {
      for (base::RegisteredLoggers::iterator it =
               ELPP->registeredLoggers()->begin();
           it != ELPP->registeredLoggers()->end(); ++it) {
    ...
    /// @brief Sets default configurations. This configuration is used for
    /// future (and conditionally for existing) loggers
    ...
    static inline const base::LogStreamsReferenceMap* logStreamsReference(
        void) {
    ...
    /// @brief Configures loggers using command line arg. Ensure you have
    /// already set command line args,
    /// @return False if invalid argument or argument with no value provided,
    /// true if attempted to configure logger.
